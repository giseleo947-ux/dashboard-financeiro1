...
fig2.add_trace(go.Bar(
    ...
    x=["Endividamento"],
    y=[endividamento_total]
))

fig2.add_trace(go.Bar(
    name="Endividamento Real",
    x=["Endividamento"],
    y=[endividamento_real]
))
...
fig5.add_trace(go.Pie(
    labels=["Estoque", "Outros Ativos"],
    values=[estoque, ativo_total - estoque]
))
...
# ATIVO TOTAL
# =========================
st.subheader("🏢 Ativo Total")

st.metric("Ativo Total", brl(ativo_total))
...
if estoque_sobre_ativo > 30:
    st.warning("Estoque elevado em relação ao ativo")